...
- 新增 Gateway/聊天流式调试链路：主进程记录 `debug trace` 并通过 IPC 向 UI 实时输出，便于定位思考与工具事件顺序问题。
- Agent 配置页支持更细粒度的模型更新同步，聊天「查看资料」可实时读取最新模型配置。

### 修复

- 修复连续工具调用场景下「思考过程」被后一次覆盖的问题，改为一个工具调用对应一个思考块并保留历史。
- 修复工具调用卡片「执行结果为空」问题，正确回填 stdout/stderr/exit code 并保留失败信息。
- 修复聊天流式展示中的气泡宽度与长命令溢出问题，避免超出消息容器。

### 变更

- 增强 UI 启动日志诊断信息：补充端口检测、依赖检查、进程就绪等待与连接重试等可见日志。
- 调整聊天窗口自动滚动策略：流式消息期间保持跟随底部，减少中途停滞。
...
- 启动流水线会通过 `gateway:log` 向渲染层输出可见的启动日志。
- 新增 `pushGatewayLog`，用于主动推送网关/启动日志并保持日志缓冲区同步。

### 修复

- 优化内置 OpenClaw 启动可观测性：Token 初始化、端口冲突、依赖检查、就绪/超时状态均有明确日志。
- README 顶部新增稳定性提醒横幅，明确当前测试版本仍不稳定。
